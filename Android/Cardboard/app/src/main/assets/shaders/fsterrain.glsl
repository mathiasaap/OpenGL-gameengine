...
vec4 grassComp = texture(GrassTexture,texToFrag*33.0f)*multitextureComponents.x;
vec4 snowComp =texture(SnowTexture,texToFrag*33.0f)*multitextureComponents.z;
vec4 rockComp=texture(RockTexture,texToFrag*8.0f)*multitextureComponents.y;
vec4 texColor= grassComp+snowComp+rockComp;


//out_color= vec4(1,1,1,1) *texture(RockTexture,texToFrag*8.0f);
out_color= texColor;
